...
function rotateSession(user) {
  const sessionId = crypto.randomBytes(16).toString('hex');
  userStore.setSessionId(user.id, sessionId);
  return sessionId;
}
...
function tokenFor(user) {
  const sessionId = rotateSession(user);
...
async function signup(email, password) {
  ...
  if (userStore.findByEmail(email)) throw httpError(409, 'An account with this email already exists. Try signing in.');
...
async function forgotPassword(email) {
  ...
  if (user) {
    ...
    userStore.setResetToken(user.id, resetToken, Date.now() + RESET_TTL_MS);
...
async function resetPassword(token, newPassword) {
  ...
  userStore.setPassword(user.id, passwordHash);
...
module.exports = { signup, login, forgotPassword, resetPassword, requireAuth, issueToken, tokenFor, httpError };
